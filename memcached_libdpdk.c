...
// Thread function to process packets through memcached
static int memcached_thread_func(void *arg)
{
    struct memcached_thread_data *data = (struct memcached_thread_data *)arg;

    while (1) {
        if (__sync_bool_compare_and_swap(&data->ready, 1, 0)) {
            pthread_mutex_lock(&data->lock);
            data->rsp_pckt_size = process_through_memcached(data->rx_buff_ptr, data->tx_buff_ptr);
...
int main (int argc, char **argv) {
    ...
    static struct memcached_thread_data memcached_data = {0};
    static int thread_initialized = 0;

    if (!thread_initialized) {
        pthread_mutex_init(&memcached_data.lock, NULL);

        // Launch memcached_thread_func on lcore 5
        unsigned lcore_id = 40;
        int ret = rte_eal_remote_launch(memcached_thread_func, &memcached_data, lcore_id);
        if (ret < 0) {
            rte_exit(EXIT_FAILURE, "Failed to launch memcached_thread_func on core %d\n", lcore_id);
        }
    ...
    while (!stop_main_loop) {
        ...
        for (int i = 0; i < received_pckt_cnt; ++i) {
            ...
            pthread_mutex_lock(&memcached_data.lock);
            memcached_data.rx_buff_ptr = rx_buff_ptr;
            memcached_data.tx_buff_ptr = tx_buff_ptr;
            memcached_data.ready = 1;
            pthread_mutex_unlock(&memcached_data.lock);

            // Wait for processing to complete
            while (__sync_val_compare_and_swap(&memcached_data.ready, 0, 0)) {
                rte_pause();
            }
            size_t rsp_pckt_size = memcached_data.rsp_pckt_size;
