...
//HUY
struct memcached_thread_params {
    uint8_t* rx_buff_ptr;
    uint8_t* tx_buff_ptr;
    size_t rsp_pckt_size;
    int work_ready;  // Flag to indicate if work is ready
    pthread_mutex_t mutex;
    pthread_cond_t cond;
};

void* memcached_thread_func(void* args) {
    struct memcached_thread_params* params = (struct memcached_thread_params*)args;
    ...
    while (1) {
        // Lock mutex and wait for the work signal
        pthread_mutex_lock(&params->mutex);
        while (!params->work_ready) {
            pthread_cond_wait(&params->cond, &params->mutex);
        }
        params->work_ready = 0;  // Reset the work flag

        // Process the data
        params->rsp_pckt_size = process_through_memcached(params->rx_buff_ptr, params->tx_buff_ptr);
...
int main (int argc, char **argv) {
    //HUY
    pthread_t thread;
    struct memcached_thread_params thread_params;
    pthread_mutex_init(&thread_params.mutex, NULL);
    pthread_cond_init(&thread_params.cond, NULL);
    thread_params.work_ready = 0;
    if (pthread_create(&thread, NULL, memcached_thread_func, &thread_params) != 0) {
    ...
    while (!stop_main_loop) {
        ...
        for (int i = 0; i < received_pckt_cnt; ++i) {
            ...
            thread_params.rx_buff_ptr = rx_buff_ptr;
            thread_params.tx_buff_ptr = tx_buff_ptr;
            thread_params.work_ready = 1;

            // Signal the worker thread to start processing
            pthread_cond_signal(&thread_params.cond);

            // Wait for the processing to complete
            while (thread_params.work_ready) {
                pthread_cond_wait(&thread_params.cond, &thread_params.mutex);
            }
            pthread_mutex_unlock(&thread_params.mutex);

            // Retrieve the response packet size
            size_t rsp_pckt_size = thread_params.rsp_pckt_size;
